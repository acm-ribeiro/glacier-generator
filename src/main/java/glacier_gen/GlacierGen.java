...
public class GlacierGen {
    ...
    /**
     * Generates the Glacier contracts for the specification
     */
            ...
            switch (verb.toUpperCase()) {
                ...
                case PUT, PATCH -> {
                    if (id != null) {
                        ...
                        op.addPostcondition(previous_put(resource, id).toString());
                    }
                }
    ...
    /**
     * Builds a Glacier formula of the type: response_code(GET /resource_name/{resource_id}) == code When uri is
     * present, resource and id should be null. When resource and id are present, uri should be null.
     *
     ...
     * @return formula
     */
    public static Formula response_code(@Nullable String resource, @Nullable String id, @Nullable String uri,
        String code) {
    ...
    /**
     * Builds a Glacier formula of the type: request_body(this) != previous(response_body(GET /resource/{id}))
     *
    ...
    /**
     * Builds a Glacier formula of the type:
     * response_body(GET /resource/{id}) != previous(response_body(GET /resource/{id}))
     *
     * @param resource resource name
     * @param id resource id
     * @return formula
     */
    public static Formula previous_put(String resource, String id) {
        // Building the uri
        String url = "/" + resource + "/{" + id + "}";

        // Building the left term
        OperationHeader l_op_header = new OperationHeader(RESPONSE_BODY);
        HTTPRequest request = new HTTPRequest(GET, url);
        OperationParameter l_parameter = new OperationParameter(request, null);
        Operation l_operation = new Operation(l_op_header, l_parameter, null, null, null);
        Call l_call = new Call(l_operation, null);
        LeftTerm left = new LeftTerm(l_call, null, false);
    ...
    /**
     * Returns all getters from a URI, when applicable. e.g., uri = "/r1/{r1_id}/r2/{r2_id}" returns ["/r1/{id1}",
     * "/r1/{r1_id}/r2/{id2}"], if there are two get methods with these URIs.
     *
     * @param uri uri
