...
public class Main {
    ...
    public static void main(String[] args) throws EncodeException, IOException {
        ...
        try {
            // Creating glacier contracts directory if it does not exist
            createDirectory();

            // Parsing OAS
            Specification spec = OASCustomParser.parse(specFile);

            // Generating glacier contracts
            ...
            e.printStackTrace();
            System.err.println("OpenAPI 3 validation failure. Please, provide a specification in version 3.0.");
    ...
    private static FileWriter createJsonFile(String filename) throws IOException {
        String glacier_filename = filename.replace(".json", "") + "-glacier.json";
        return new FileWriter(DIRECTORY + "/" + glacier_filename);
    }
